...
SELECT TGENEROS.NOME_GEN,SUM(PUBLICO) FROM TSESSOES,TGENEROS WHERE TSESSOES.ID_GEN=TGENEROS.ID_GEN 
GROUP BY TGENEROS.NOME_GEN ORDER BY(2) DESC;
...
SELECT TPESSOAS.NOME_PES, SUM(TSESSOES.PUBLICO)"PUBLICO" FROM TSESSOES,TPESSOAS WHERE TSESSOES.ID_PES=TPESSOAS.ID_PES 
GROUP BY TPESSOAS.NOME_PES ORDER BY "PUBLICO" DESC;
...
SELECT TCINEMA.NOME_CIN FROM TSESSOES,TCINEMA WHERE TSESSOES.ID_CIN IN 
(SELECT TMP.ID_CIN FROM (SELECT ID_CIN,MAX(ARRECADACAO) TOTAL FROM TSESSOES GROUP BY ID_CIN )TMP);
